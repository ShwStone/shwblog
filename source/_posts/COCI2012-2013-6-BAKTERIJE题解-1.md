...
考虑用 `book[x][y][d]` 来表示第一次到达位置 $(x, y, d)$ 所需的步数。在模拟的过程中，如果发现重复进入同一个位置（即 `book[x][y][d] != 0` ），说明产生了循环，循环节长度为 `当前步数 - book[x][y][d]` ，记为 `stp(step)` ，进入循环节之前的步数即为 `book[x][y][d]` ， 记为 `st(start)`。

由于题目要求所有细菌进入陷阱，所以我们还要统计每一个细菌进入陷阱的时刻。用 `pas[i]` 表示细菌第一次以方向 `i` 进入陷阱的时刻。可以发现，如果 `pas[i] < st` ，则细菌只可能在进入循环节之前以方向 `i` 进入陷阱，在循环节中绝对不会（如果也进了说明 `st` 可以提前到 `pas[i]`）。同理，在循环节中经过的也不会在循环节之前经过。所以用一个数组就可以记录，互不干扰。
...
接下来要把问题分为两部分。求出最晚进入循环的细菌进入循环的时刻，记为 `mst(max start)` ，则在 `mst` 之前的时刻可以暴力枚举，在 `mst` 之后的时间可以用数学方法。

在 `mst` 之前的时刻枚举如下，注意此时已经有一些细菌进入循环，所以也要考虑进入循环的 `pas[i]` 。
